  ...
  })(
    {
      ...
      4: [
        function(_dereq_, module, exports) {
          ...
          function renderWindow(
            ...
            var width =
              options.type === "view" ? windowWidth : bounds.right + 1;
            var height =
              options.type === "view" ? windowHeight : bounds.bottom + 1;
        ...
        function(_dereq_, module, exports) {
          ...
          function matrixInverse(m) {
            ...
            var a = m[0],
              ...
              f = m[5];
            var det = a * e - b * d;
            var M = [e, -d, -b, a, b * f - c * e, c * d - a * f].map(function(
              val
            ) {
              return val / det;
            });
      ...
      15: [
        function(_dereq_, module, exports) {
                ...
                switch (borderSide) {
                  case 0:
                    ...
                    border.pathArgs = drawSidePath(
                      ...
                      radius[0],
                      radius[1],
                      borderPoints.topLeft,
                      borderPoints.topRight
                    );
                    break;
                  case 1:
                    // right border
          ...
          function calculateCurvePoints(bounds, borderRadius, borders) {
            ...
            return {
              topLeft: getCurvePoints(
                x + borders[3].width / 2,
                y + borders[0].width / 2,
                Math.max(0, tlh - borders[3].width / 2),
                Math.max(0, tlv - borders[0].width / 2)
              ).topLeft.subdivide(0.5),
              topRight: getCurvePoints(
                x + Math.min(topWidth, width + borders[3].width / 2),
                y + borders[0].width / 2,
                topWidth > width + borders[3].width / 2
                  ? 0
                  : trh - borders[3].width / 2,
                trv - borders[0].width / 2
              ).topRight.subdivide(0.5),
              bottomRight: getCurvePoints(
                x + Math.min(bottomWidth, width - borders[3].width / 2),
                y + Math.min(rightHeight, height + borders[0].width / 2),
                Math.max(0, brh - borders[1].width / 2),
                brv - borders[2].width / 2
              ).bottomRight.subdivide(0.5),
              bottomLeft: getCurvePoints(
                x + borders[3].width / 2,
                y + leftHeight,
                Math.max(0, blh - borders[3].width / 2),
                blv - borders[2].width / 2
      ...
      20: [
        function(_dereq_, module, exports) {
          ...
          CanvasRenderer.prototype.shadow = function(shape, shadows) {
            var parseShadow = function(str) {
              ...
              for (var key in propertyFilters) {
                info[key] = properties.filter(
                  propertyFilters[key].test.bind(propertyFilters[key])
                );
                info[key] =
                  info[key].length === 0
                    ? null
          ...
          CanvasRenderer.prototype.mask = function(
            shapes,
            callback,
            context,
            container
